...
import PayslipComponent from './slip_gaji';
...
const DataPayrollKaryawan = () => {
    ...
    const [direkturList, setDirekturList] = useState<any[]>([]);
    const [showDialog, setShowDialog] = useState(false);
    const [showDialogSlip, setShowDialogSlip] = useState(false);
    ...
    const fetchPayrollData = async () => {
        ...
        try {
            const response = await axios.get('http://127.0.0.1:8000/api/payroll-summary', {
                headers: {
    ...
    const handleSubmit = async () => {
        ...
        try {
            await axios.post('http://127.0.0.1:8000/api/payroll-store', mappedData, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleEdit = (payroll: PayrollData) => {
        setEditRequest({
            ...
            id_direktur: '', // You might need to fetch this from the payroll data
            namaKaryawan: payroll.namaKaryawan,
            tanggal: payroll.tanggal ? new Date(payroll.tanggal) : new Date(),
            target_absensi: 0, // You might need to fetch these from the payroll data
    ...
    const handleUpdate = async () => {
        ...
        try {
            await axios.put(`http://127.0.0.1:8000/api/payroll-update/${editRequest.id}`, mappedData, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            ...
            if (result.isConfirmed) {
                await axios.delete(`http://127.0.0.1:8000/api/payroll-delete/${id}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                ...
                toastRef.current?.show({
                    severity: 'success',
                    summary: 'Berhasil',
                    detail: 'Data payroll berhasil dihapus'
                });
            }
        } catch (error) {
            ...
            toastRef.current?.show({
                severity: 'error',
                summary: 'Error',
                detail: 'Gagal menghapus data payroll',
                life: 3000
            });
        }
    };
        ...
        <div className="datatable-templating-demo">
            ...
            <div className="card">
                ...
                <DataTable value={payrolls} responsiveLayout="scroll" loading={loading} paginator rows={10} rowsPerPageOptions={[5, 10, 25]}>
                    ...
                    <Column field="tunjangan" header="Tunjangan" body={(data) => `Rp ${data.tunjangan.toLocaleString('id-ID')}`} style={{ width: '10%' }} />
                    <Column field="bonus" header="Bonus" body={(data) => `Rp ${data.bonus.toLocaleString('id-ID')}`} style={{ width: '10%' }} />
                    <Column field="potongan" header="Potongan" body={(data) => `Rp ${data.potongan.toLocaleString('id-ID')}`} style={{ width: '10%' }} />
                    <Column field="totalgaji" header="Total Gaji" body={(data) => `Rp ${data.totalgaji.toLocaleString('id-ID')}`} style={{ width: '10%' }} />
                    <Column
                        field="slipGaji"
                        header="Slip Gaji"
                        body={(data) => (
                ...
                {/* Add Dialog */}
                <Dialog header={'Tambah Payroll'} visible={dialogVisible} style={{ width: '50vw' }} onHide={() => setDialogVisible(false)}>
                    <div className="p-fluid">
                        ...
                        {/* <div className="field">
                            <label htmlFor="id_direktur">Direktur</label>
                            <Dropdown
                                value={request.id_direktur}
                                options={karyawanList}
                                itemTemplate={(item) => (
                                    <>
                                        {item.nama_karyawan} ({item.jabatan})
                                ...
                                optionValue="id"
                                optionLabel="nama_karyawan"
                                onChange={(e) => setRequest({ ...request, id_direktur: e.value })}
                        ...
                        {/* <div className="field">
                            <label htmlFor="target_produktivitas">Target Produktivitas</label>
                            <InputNumber
                                id="target_produktivitas"
                                value={request.target_produktivitas}
                                onValueChange={(e) => setRequest({ ...request, target_produktivitas: e.value })}
                                mode="decimal"
                                min={0}
                                showButtons
                                placeholder="Masukkan Target Produktivitas"
                                className="w-full"
                            />
                        </div> */}
                ...
                {/* Edit Dialog */}
                <Dialog header={'Edit Payroll'} visible={editDialogVisible} style={{ width: '50vw' }} onHide={() => setEditDialogVisible(false)}>
                    <div className="p-fluid">
                        <div className="field">
                            <label htmlFor="id_karyawan">Karyawan</label>
                            <Dropdown
                        ...
                        {/* <div className="field">
                            ...
                            <Dropdown
                                ...
                                optionValue="id"
                                optionLabel="nama_karyawan"
                                onChange={(e) => setEditRequest({ ...editRequest, id_direktur: e.value })}
                                placeholder="Pilih Direktur"
                            />
                        </div> */}
                        ...
                        {/* <div className="field">
                            ...
                            <InputNumber
                                id="target_produktivitas"
                                value={editRequest.target_produktivitas}
                                onValueChange={(e) => setEditRequest({ ...editRequest, target_produktivitas: e.value })}
                                mode="decimal"
                                min={0}
                                showButtons
                                placeholder="Masukkan Target Produktivitas"
            ...
            <PayslipComponent setShowDialogSlip={setShowDialogSlip} showDialogSlip={showDialogSlip} employeeData={selectedPayroll} />
        </div>
    );
};
